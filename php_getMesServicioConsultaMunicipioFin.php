<?php
    ...
    // Query
    if ($tipo == "p_diario") {
        $query  = "SELECT extract(month from estado".$estado."diarios.fecha) as mes from estado".$estado."diarios inner join estaciones on estado".$estado."diarios.numero=estaciones.numero where estaciones.municipioid=".$municipio." and extract(year from estado".$estado."diarios.fecha) =".$anioFin." group by extract(month from estado".$estado."diarios.fecha) order by extract(month from estado".$estado."diarios.fecha) asc";
    }elseif ($tipo == "v_min") {
        $query  = "SELECT extract(month from estado".$estado.".fecha) as mes from estado".$estado." inner join estaciones on estado".$estado.".numero=estaciones.numero where estaciones.municipioid=".$municipio." and extract(year from estado".$estado.".fecha) =".$anioFin." group by extract(month from estado".$estado.".fecha) order by extract(month from estado".$estado.".fecha) asc";
    }
